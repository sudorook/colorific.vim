...
def _create_dicts():
    ...
    rgb2short_dict = {}
    for key, val in short2rgb_dict.items():
        rgb2short_dict[val] = key
    return rgb2short_dict, short2rgb_dict


def short2rgb(short):
    """Convert short code to rgb."""
...
def rgb2short(rgb):
    ...
    for part in parts:
        i = 0
        while i < len(incs) - 1:
            small, big = incs[i], incs[i + 1]  # smaller, bigger
            if small <= part <= big:
                small1 = abs(small - part)
                big1 = abs(big - part)
                if small1 < big1:
                    closest = small
                else:
...
if __name__ == "__main__":
    ...
    if len(arg) < 4 and int(arg) < 256:
        rgb_code = short2rgb(arg)
        sys.stdout.write(
            "xterm color \033[38;5;%sm%s\033[0m -> RGB exact \033[38;5;%sm%s\033[0m"
            % (arg, arg, arg, rgb_code)
        )
        sys.stdout.write("\033[0m\n")
    else:
        short_code, rgb_code = rgb2short(arg)
        sys.stdout.write(
            "RGB %s -> xterm color approx \033[38;5;%sm%s (%s)"
            % (arg, short_code, short_code, rgb_code)
        )
        sys.stdout.write("\033[0m\n")
